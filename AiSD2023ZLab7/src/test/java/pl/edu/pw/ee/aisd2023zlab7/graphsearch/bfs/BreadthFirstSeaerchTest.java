...
public class BreadthFirstSeaerchTest {
    ...
    public void should_ReturnBfsResult_when_GraphIsDisconnected() {
        ...
        //System.out.println(canReachBfromA(PATH_GRAPH_9_9, 6, 8));
    ...
    public boolean canReachBfromA(String pathToGraphDataFile, int verticeA, int verticeB) {
        boolean isReachable;
        Graph graph = new AdjacencyMatrix(pathToGraphDataFile);

        GraphBfsResult result = (GraphBfsResult) graphSearch.searchGraphPaths(graph, verticeA);

        if(result.getDistanceAtIndex(verticeB) != MAX_VALUE) {
            isReachable = true;
        } else {
            isReachable = false;
        }
