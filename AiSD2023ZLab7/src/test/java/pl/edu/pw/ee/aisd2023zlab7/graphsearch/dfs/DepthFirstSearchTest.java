...
public class DepthFirstSearchTest {
    ...
    public boolean isGraphConnected(GraphDfsResult result) {
        int negativeValCounter = 0;
        int [] prevVertices;
        boolean isConnected;

        prevVertices = result.getPrevVertices();
        for(int i = 0; i < prevVertices.length; i++){
            if(prevVertices[i] == -1) {
                negativeValCounter++;
            }
        }
        isConnected = negativeValCounter == 1; //jesli nie jest 1, to musieliśmy znowu wejsc gdzies z nikąd
